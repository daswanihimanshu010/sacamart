...
class DataController extends Controller
{
    ...
    {
        $lantypedd = LoanUserDocumentType::where('loan_type_id',$request->loan_type)->get();
        $loantypebytype = array();
        foreach($lantypedd as $singlelantype){
            $loantypebytype[] = $singlelantype->user_type;
        }
        //prd($loantypebytype);
        ...
        $asdflasdfjlasj = array();
		foreach($fields as $singlefield){
            if($singlefield->title=='Type Of Employment'){
                $singlefield->options_value = json_encode($loantypebytype);
            }
        $asdflasdfjlasj[] = $singlefield;
        }
		//prd($asdflasdfjlasj);die;
...
//		->first();
	...
	public function get_doc_list_array($loanfields,$loanid=0,$folder=0){
		...
		foreach($docgroup as $singledocgroup){
            ...
            $docfifields = implode(',',$docsetlistarr);
            $docarray = DocumentField::whereIn('id', $docsetlistarr)->orderByRaw("FIELD(id, $docfifields)")->get();
            //$docarray = DocumentField::whereIn('id', $docsetlistarr)->orderByRaw("FIELD(id, $docsetlistarr)")->get();
...
//        $data = $this->createRule($rules, $fields, $request);
    ...
    function update_loan_bank_status(Request $request){
       $validator = Validator::make($request->all(), [
		...
		if($loan->save()){
             ...
             if($loanoldstatus!=$request->loan_status){
                
                $loanbankdata = LoanBank::where('id', $request->bank_id)->first();
                $loandata = Loan::with('borrower_detail')->where('id',$loanbankdata->loan_id)->first();
                $fcmtoken = $loandata->borrower_detail->fcm_token;
                $LoanStatus = LoanStatus::find($request->loan_status);
                //prd($LoanStatus);
                $tokenarray = array($fcmtoken);
                $title = 'Loan status changed';
                $description = 'your loan file ('.$loanbankdata->loan_id.') status has been changed to '.$LoanStatus->title;
                send_notification($title,$description,'',$tokenarray = array(),'loan',$loanbankdata->loan_id);
            }
    ...
    public function notificationlist(Request $request)
    {
        ...
        if ($validator->fails()) {
		   return response()->json([
			'status' => false,
			'errors' => 'Please send type:user/emp and user_id'
			]);
        }
        $type = $request->type;
        $user_id = $request->user_id;
		$bannerlist = Notification::where('type', $type)->orWhere(['user_id'=>0,'user_id'=>$user_id])->get();
        $bannerarray = array();
        foreach($bannerlist as $singlebanner){
            $singlebannern = array();
            $singlebannern['id'] = $singlebanner->id; 
            $singlebannern['title'] = $singlebanner->title; 
            $singlebannern['image'] = $singlebanner->banner; 
            $singlebannern['content'] = $singlebanner->description;
            $singlebannern['created_at'] = $singlebanner->created_at->format('Y-m-d H:i:s');
            $bannerarray[] = $singlebannern;
        }
        return response()->json([
            'status' => true,
