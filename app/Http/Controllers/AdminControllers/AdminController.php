...
class AdminController extends Controller
{
	...
	public function dashboard(Request $request){
		...
		//recently order placed
         if(Auth::user()->role_id==14){
            $myorderids = $this->getvendororderids(Auth::user()->id);
             //dd($myorderids);die;
              $orders = DB::table('orders')
			->LeftJoin('currencies', 'currencies.code', '=', 'orders.currency')
			->where('customers_id','!=','')
			->whereIn('orders_id',$myorderids)
		...
		$result['total_orders'] = count($orders);
         ...
         if(Auth::user()->role_id==14){
            $productsids = $this->getvenderproducts(Auth::user()->id);
            ...
            $recentProducts = DB::table('products')
                ...
                ->whereIn('products.products_id', $productsids)
        ...
        if(Auth::user()->role_id==14){
            ...
            if($vendors->package_expiery_date > time()){
                $packagelist = DB::table('packages')->where('id',$vendors->package_id)->get();
                //print_r($vendors->package_id);die;
            }else{
  ...
  public function checkLogin(Request $request){
		...
		if($validator->fails()){
			...
			if(auth()->attempt($adminInfo)) {
				...
				$administrators = DB::table('users')->where('id', $admin->myid)->where('status', 1)->get();
  ...
  //admins
	...
	public function addadmins(Request $request){
		...
		return view("admin.admins.add",$title)->with('result', $result)->with('country',$country);
  ...
  //update admin
   ...
   public function profile(Request $request){
        ...
        $zones = DB::table('states')->where('country_id', '=',101)->get();
        $result['countries'] = $countries;
        $result['zones'] = $zones;
		$result['commonContent'] = $this->Setting->commonContent();
  ...
  //editCategoriesRoles
    ...
    function getvendororderids($user_id=0){
    ...
    function getvenderproducts($user_id=0){
         $product = DB::table('products')->where('user_id', '=', $user_id)->get();
         $userporudctsarray = array(0);
         foreach($product as $singleprd){
            $userporudctsarray[] =$singleprd->products_id;  
