...
class SiteSettingController extends Controller
{
    ...
    public function notifyme()
    {
        ...
        $notifyme = $this->Setting->notifyme();
        
        $result['notifyme'] = $notifyme;
        $result['commonContent'] = $this->Setting->commonContent();

        /*foreach ($notifyme as $key => $notify) {
            $request['products_id'] = $notify->products_id;
            $request['attributeid'] = 0;

            $result['currentStock'] = $this->products->currentstock($request);
        }*/

        return view("admin.products.notifyme")->with('result', $result);
    }
//alert Setting
