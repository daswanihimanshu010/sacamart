...
class OrdersController extends Controller
{
    ...
    //add listingOrders
    public function display($type='all',Request $request)
    {
        //$language_id = '1';
        ...
        $ordersData['orders'] = $this->Order->paginator($type,$request);
        $ordersData['message'] = $message;
        $ordersData['errorMessage'] = $errorMessage;
        $ordersData['currency'] = $this->myVarsetting->getSetting(); 
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.Orders.index", $title)->with('listingOrders', $ordersData)->with('result', $result)->with(['countrylist' => $countrylist, 'statelist' => $statelist, 'citylist' => $citylist, 'locations' => $locations]);
    }
    ...
    public function invoiceprint(Request $request)
    {
        ...
        foreach ($order as $data) {
            ...
            $orders_products = DB::table('orders_products')
                ...
                ->select('orders_products.*', 'products.products_image as image', 'tax_rates.tax_rate')
