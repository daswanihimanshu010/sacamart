...
class AlertController extends Controller
{
	...
	public function orderAlert($ordersData){
		...
		if($alertSetting[0]->order_email==1){
			...
			//admin email
			if(!empty($ordersData['orders_data'][0]->vendor_email)){
				try {
				Mail::send('/mail/orderEmail', ['ordersData' => $ordersData], function($m) use ($ordersData){
					$m->to($ordersData['orders_data'][0]->vendor_email)->subject(Lang::get("labels.Sacamart: An order has been placed"))->getSwiftMessage()
					->getHeaders()
					->addTextHeader('x-mailgun-native-send', 'true');
				});
			} catch (\Exception $e) {
			...
			if(!empty($ordersData['orders_data'][0]->email)){
				try {
					...
					Mail::send('/mail/orderEmail', ['ordersData' => $ordersData], function($m) use ($ordersData){
						$m->to($ordersData['orders_data'][0]->email)->subject(Lang::get("labels.Sacamart: An order has been placed"))->getSwiftMessage()
						->getHeaders()
						->addTextHeader('x-mailgun-native-send', 'true');
					});
			...
			if(!empty($ordersData['orders_data'][0]->delivery_phone)){

				$msg = "Your Order No ".$ordersData['orders_data'][0]->orders_id." has been received.";
		        ...
		        curl_setopt_array($curl, array(
		          ...
		          CURLOPT_POSTFIELDS => "userId=sacmrt&password=Sacamart@1!&mobile=91".$ordersData['orders_data'][0]->delivery_phone."&msg=".$msg."&senderId=sacmrt&msgType=text&duplicateCheck=true&format=json&sendMethod=simpleMsg",
