...
class CartController extends Controller
{
    ...
    public function viewcart(Request $request)
    {
        ...
        //echo '<pre>'; print_r($result['cart']); exit;
        //apply coupon
        if (session('coupon')) {
    ...
    { 
       ...
       $customers_basket_id = $request->customers_basket_id; 
        ...
        $session_id = Session::getId();
        $this->cart->add_note($customers_basket_id, $customers_id, $session_id, $note);

        $message = Lang::get("Note has been Added successfully");
        return Response::json(['status' => true, 'mes' => $message]);
    ...
    public function getShipCharge(){
        ...
        { 
         ...
         /*$your_area = @$_SESSION["your_area"];
         $proData = Products::where('products_id',$proId)->get()->toArray();
         $vId = $proData[0]['user_id'];
         
         $vendordetail = DB::table('vendors')->where('user_id',$vId)->get();

         $results =DB::table('vendors_location')->where('location_name',$your_area)->where('vendor_id',$vendordetail[0]->id)->get();

         
         $shippingFee = @$results[0]->shipping_fees;
         $min_order = @$results[0]->min_order;*/
         
         /*return array('shipping_fees'=>$shippingFee);*/
    }
    
    public function current_state($stateId=null){
        $location = States::where('id',$stateId)->get();
        echo @$location[0]->name;
    }
    public function current_city($cityId=null){
        $location = City::where('id',$cityId)->get();
        echo @$location[0]->name;
    }
    public function current_location($areaId=null){
        $location = Location::where('id',$areaId)->get();
