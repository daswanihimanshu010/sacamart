...
class OrdersController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        if (count($result['cart']) == 0) {
            ...
            $result['zones'] = DB::table('states')->where('country_id','101')->orderBy('name','asc')->get();
            //$result['statelist'] = DB::table('states')->where('country_id','101')->orderBy('name','asc')->get();
            $result['citylist'] = DB::table('cities')->get();
            $result['locations'] = DB::table('locations')->get();
            //print_r($result);die;
    ...
    public function checkout_shipping_address(Request $request)
    {
        ...
        if (empty(session('billing_address')) or session('billing_address')->same_billing_address == 1) {
            ...
            $order_weight = session('total_shipping_weight');

            $shipping_cost_data =DB::table('locations')->leftJoin('shipping_weight_slabs', 'shipping_weight_slabs.shipping_zone_id', '=', 'locations.shipping_zone_id')->where('locations.id',$billing_data['billing_zip'])->where('shipping_weight_slabs.range_start','<=',$order_weight)->where('shipping_weight_slabs.range_end','>=',$order_weight)->get();

            $shippingFee = json_decode($shipping_cost_data, true)[0]['shipping_fee'];
            //$shippingFee = 0;

            session(['billing_address' => $billing_address]);
            session(['shipping_charges' => $shippingFee]);
    ...
    public function shipping_methods()
    {
        ...
        if (!empty(session('shipping_address'))) {
            $countries_id = session('shipping_address')->countries_id;
            $toPostalCode = session('shipping_address')->entry_postcode;
            $toCity = session('shipping_address')->entry_city;
            $toAddress = 'gh';
