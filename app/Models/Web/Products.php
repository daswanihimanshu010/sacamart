...
class Products extends Model
{
    ...
    protected $fillable = [
        'user_id', 'products_quantity', 'products_model', 'products_image', 'products_video_link', 'products_price', 'products_date_added'
, 'products_last_modified', 'products_date_available', 'products_weight', 'products_weight_unit', 'products_status', 'is_current', 'products_tax_class_id', 'manufacturers_id', 'products_ordered', 'products_liked', 'low_limit', 'is_feature', 'products_slug', 'products_type', 'products_min_order', 'products_max_stock', 'currency'];

    
    public function vendor()
    {
        //return $this->belongsTo('App\Job','job_category_id');
        return $this->belongsTo('App\Models\Web\Users','user_id','id');
    }
    
    public function vendor_location() {
        return $this->hasMany('App\Models\Web\Vendor_location','location_name');
    }
...
///////////////////////////////////////////////////////////////////////////////////////////////
    ...
    /*public function vendors($data) {
        $vendor_id = $data['vendor_detail_id'];
        $type = $data['type'];
    ...
    public function products($data)
    {
        ...
        if (!empty($data['search'])) {
            ...
            if (!empty($data['vendor_id'])) {
                $categories->where('products.user_id', '=', $data['vendor_id']);
            }
            ...
            if (!empty($data['filters'])) {
                ...
                foreach ($data['filters']['filter_attribute']['option_values'] as $option_id_temp) {
                    ...
                    if ($temp_key == 0) {
                        ...
                        $categories->whereIn('products_attributes.options_id', [$data['filters']['options']])->where('products_attributes.options_values_id', $option_id_temp);
                        if (count($data['filters']['filter_attribute']['options']) > 1) {
        ...
        if (!empty($data['filters'])) {
            ...
            foreach ($data['filters']['filter_attribute']['option_values'] as $option_id_temp) {
                ...
                if ($temp_key == 0) {
                    ...
                    $categories->orwhereIn('products_attributes.options_id', [$data['filters']['options']])->where('products_attributes.options_values_id', $option_id_temp);
        ...
        $productsbyloc = array();
        if(!empty($_SESSION["your_area"])){
           $productsbyloc =$this->findvendorwithloc();
           
         
            
             //print_r($productsbyloc);
//            print_r($vendors);die;
//            $useridsforprd = array(09283);
//           foreach($vendors as $singlevendor){
//             $useridsforprd[] =  $singlevendor->user_id; 
//           }
//            print_r($useridsforprd);die;
           $categories->whereIn('products.user_id',$productsbyloc);
        }
        
        if(isset($data['requestdata']['Vendor']) && !empty($data['requestdata']['Vendor'])){
          
            $categories->whereIn('products.user_id',$data['requestdata']['Vendor']);
        }
        if(isset($data['requestdata']['vendorid']) && !empty($data['requestdata']['vendorid'])){
          
            $categories->where('products.user_id',$data['requestdata']['vendorid']);
        }
        ...
        if (count($products) > 0) {
            ...
            foreach ($products as $products_data) {
                ...
                //product vendor    
                 $products_vendor = DB::table('products')
                    ->LeftJoin('users', 'products.user_id', '=', 'users.id')
                    ->select('users.first_name as vendor_name')
                    ->where('products_id', '=', $products_id)
                    ->first();        

                $products_data->vendor = $products_vendor;

                $products_vendor_info = DB::table('products')
                    ->LeftJoin('users', 'products.user_id', '=', 'users.id')
                    ->select('users.*')
                    ->where('products_id', '=', $products_id)
                    ->first();        

                $products_data->vendor_info = $products_vendor_info;

                $products_vendor_images = DB::table('products')
                    ->LeftJoin('vendors', 'products.user_id', '=', 'vendors.user_id')
                    ->select('vendors.*')
                    ->where('products_id', '=', $products_id)
                    ->first();        

                $products_data->vendor_image_info = $products_vendor_images;
                //echo '<pre>'; print_r($products_data->vendor); exit();
    ...
    public function cartIdQuantity($requestt)
    {
        ...
        $index = 0;
        foreach ($baskit as $baskit_data) {
            $result[$baskit_data->products_id] = $baskit_data->customers_basket_quantity;
        }
    ...
    {
        $category = DB::table('categories')->leftJoin('categories_description', 'categories_description.categories_id', '=', 'categories.categories_id')->where('categories_slug', $request->category)
        ->where('categories.categories_status', 1)->where('language_id', Session::get('language_id'))->get();
        return $category;
    }
    ...
    public function getMainCategories($category)
    {
        ...
        ->where('categories.categories_status', 1)->where('categories.categories_id', $category)->where('language_id', Session::get('language_id'))->get();
    ...
    function findvendorwithloc(){
         ...
         foreach($product as $singleprd){
            $userporudctsarray[] =$singleprd->vendor_id;  
         }
         $vendors = DB::table('vendors')->whereIn('id',$userporudctsarray)->where('deleted_at',0)->where('status',1)->get();
        //print_R($vendors);die;
        $userids = array(2231);
        foreach($vendors as $singleprd){
            $userids[] =$singleprd->user_id;  
         }
        //print_r($userids);die;
         return $userids;
