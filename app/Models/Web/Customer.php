...
class Customer extends Model
{
    ...
    public function notifyProduct($request)
    {

        if (!empty(auth()->guard('customer')->user()->id)) {
            $notify_products_id = $request->products_id;

            $notify_customers_id = auth()->guard('customer')->user()->id;
            $date_liked = date('Y-m-d H:i:s');
            ...
            $total_notify = 0;
            if (!empty(session('customers_id'))) {
                $total_notify = DB::table('notify_products')
                    ->leftjoin('products', 'products.products_id', '=', 'notify_products.notify_products_id')
                    ->where('products_status', '1')
                    ->where('notify_customers_id', '=', session('customers_id'))->count();
            }
            

            $responseData = array('success' => '2', 'message' => "You will be notified when this product is in stock.", 'total_likes' => $total_notify, 'id' => 'like_count_' . $notify_products_id, 'total_wishlist' => $total_notify);

            
        } else {
            $responseData = array('success' => '0', 'message' => "Please login first to get notification for this product.");
        }
        $cartResponse = json_encode($responseData);
        return $cartResponse;
    }
    ...
    public function processOtpLogin($request, $old_session)
    {
        $result = array();
        $customer = auth()->guard('customer')->user();
        session(['guest_checkout' => 0]);
