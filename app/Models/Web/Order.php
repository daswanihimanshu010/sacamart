...
class Order extends Model
{
    ...
    public function place_order($request)
    {
        ...
        $city_info = DB::table('cities')->where('id', '=', session('shipping_address')->entry_city)->get();

        $delivery_city = json_decode($city_info, true)[0]['name'];
        
        $location_info = DB::table('locations')->where('id', '=', session('shipping_address')->entry_postcode)->get();

        $delivery_postcode = json_decode($location_info, true)[0]['location'];
        $delivery_phone = session('shipping_address')->delivery_phone;

        $delivery = DB::table('states')->where('id', '=', session('shipping_address')->zone_id)->get();
        ...
        $billing = DB::table('states')->where('id', '=', session('billing_address')->billing_zone_id)->get();
        ...
        if (session('shipping_charges') != null) {
            $shipping_cost = session('shipping_charges');
        } else {
            $shipping_cost = 0;
        }
...
//echo 2222;die;
            ...
            foreach ($order as $data) {
                ...
                $orders_products = DB::table('orders_products')
                    ...
                    ->select('orders_products.*', 'products.products_image as image', 'users.email as vendor_email')
...
//echo $payment_status;die;
    ...
    public function mostVendorOrders($orders_data)
    {
        $mostOrdered = DB::table('orders_products')
