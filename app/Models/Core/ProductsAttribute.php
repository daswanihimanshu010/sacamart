...
class ProductsAttribute extends Model
{
    ...
    public function display($request,$user_id=0){
        //echo $user_id;die;
      $extensions =  $this->myVarsetting->imageType();
        if($user_id==0){
            $attributes = DB::table('products_options')->get();
        }else{
           $attributes = DB::table('products_options')->where('user_id',$user_id)->get(); 
        }
      ...
      foreach($languages as $languages_data){
          ...
          if($i==0){
              ...
              if(Auth::user()->role_id==14){
                    //$result['attributes'] = $this->productsattribute->display($request, Auth::user()->id);
    ...
    public function deleterecord($request){
      $option_id = $request->option_id;
        ...
        if(Auth::user()->role_id==14){
                      $editoptions = DB::table('products_options')
                      ->where('products_options_id', $request->option_id)
                      ->where('user_id', Auth::user()->id)
                      ->get();
                        if($editoptions->count()<1){
                            echo 'invalid request'; exit;
