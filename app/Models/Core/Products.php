...
class Products extends Model
{
	...
	public function paginator($request,$user_id=0){
...
//        }
        ...
        if (isset($_REQUEST['categories_id']) and !empty($_REQUEST['categories_id'])) {
         ...
         if(isset($request->product) && !empty($request->product)){
                ...
                ->where('categories_status', '1')
                ->where('is_current', '1')
                ->where('products_description.products_name','like', '%'.$request->product.'%')
                ->groupBy('products.products_id')->paginate($commonsetting['pagination']);
        }
        ...
        if(isset($request->vendor_id) && !empty($request->vendor_id)){
          $products = $data->orderBy('products.products_id', 'DESC')
                ->where('categories_status', '1')
                ->where('is_current', '1')
                ->where('products.user_id', $request->vendor_id)
                ->groupBy('products.products_id')->paginate($commonsetting['pagination']);
        }

        if(isset($request->country_id) && !empty($request->country_id)){

                $counrtuserids = DB::table('vendors_location')->where('country_id', $request->country_id)->groupBy('vendor_id')->where('deleted_at', 0)->get();
                ...
                ->where('categories_status', '1')
                ->where('is_current', '1')
                ->whereIn('products.user_id', $counrtuseridsarray)
                ->groupBy('products.products_id')->paginate($commonsetting['pagination']); 
            }

            if(isset($request->state_id) && !empty($request->state_id)){

                $counrtuserids = DB::table('vendors_location')->where('state_id', $request->state_id)->groupBy('vendor_id')->where('deleted_at', 0)->get();
            ...
            if(isset($request->city_id) && !empty($request->city_id)){
                ...
                $counrtuserids = DB::table('vendors_location')->where('city_id', $request->city_id)->groupBy('vendor_id')->where('deleted_at', 0)->get();
            ...
            if(isset($request->location_name) && !empty($request->location_name)){
                ...
                $stateuserids = DB::table('vendors_location')->where('location_name', $request->location_name)->groupBy('vendor_id')->where('deleted_at', 0)->get();
                ...
                foreach ($stateuserids as $counrtuseridskey => $counrtuseridsvalue) {
                    ...
                    ->where('categories_status', '1')
                    ->where('is_current', '1')
                    ->whereIn('products.user_id', $stateuseridsarray)
                    ->groupBy('products.products_id')->paginate($commonsetting['pagination']); 
  ...
  public function insert($request){
    ...
    foreach($languages as $languages_data){
        ...
        $productuniqueno = "";
        $productuniqueno .= $user_id;
        if(!empty($request->categories)){
            $productuniqueno .= '-'.$request->categories[0];
        }
        $productuniqueno .= '-'.$products_id;
        ...
        $req_products_length = $request->products_length ;
        $req_products_breadth = $request->products_breadth ;
        $req_products_height = $request->products_height ;
        $req_products_weight = $request->shipping_weight ;
        ...
        DB::table('products_description')->insert([
            ...
            'products_length' => $req_products_length,
            'products_breadth' => $req_products_breadth,
            'products_height' => $req_products_height,
            'shipping_weight' => $req_products_weight,
    ...
    foreach($result['languages'] as $languages_data){
      $description = DB::table('products_description')
        ...
        if(count($description)>0){
            ...
            $description_data[$languages_data->languages_id]['products_length'] = $description[0]->products_length;
            $description_data[$languages_data->languages_id]['products_breadth'] = $description[0]->products_breadth;
            $description_data[$languages_data->languages_id]['products_height'] = $description[0]->products_height;
            $description_data[$languages_data->languages_id]['shipping_weight'] = $description[0]->shipping_weight;
  ...
  public function updaterecord($request){
          ...
          if($request->image_id !== null){
            ...
            if(!empty($uploadImage)){
               $dataupdatearray['products_image']  = $uploadImage;
            }
          DB::table('products')->where('products_id', '=', $products_id)->update($dataupdatearray);
          foreach($languages as $languages_data){
              $products_name = 'products_name_'.$languages_data->languages_id;
              $products_length = 'products_length_'.$languages_data->languages_id;
              $products_height = 'products_height_'.$languages_data->languages_id;
              $shipping_weight = 'shipping_weight_'.$languages_data->languages_id;
              $products_breadth = 'products_breadth_'.$languages_data->languages_id;
              $products_url = 'products_url_'.$languages_data->languages_id;
              $products_description = 'products_description_'.$languages_data->languages_id;
              //left banner
              $products_left_banner = 'products_left_banner_'.$languages_data->languages_id;
              ...
              if(count($checkExist)>0){
                  $req_products_name = $request->$products_name;
                  $products_length = $request->$products_length;
                  $products_breadth = $request->$products_breadth;
                  $products_height = $request->$products_height;
                  ...
                  ->where('language_id', '=', $languages_data->languages_id)->update([
                      ...
                      'products_breadth' => $products_breadth,
  ...
  public function addinventoryfromsidebar($user_id=0){
    $setting = new Setting();
    $myVarsetting = new SiteSettingController($setting);
    $myVaralter = new AlertController($setting);
    $language_id      =   '1';
    ...
    if($user_id>0){
        $productsidsddd = $this->getuservenderproducts($user_id);
        //print_r($productsids);die;
         $product =  $product->whereIn('products.products_id', $productsidsddd)->get();
        //print_r($product);die;
    }else{
       $product =  $product->get();  
    }
  ...
  public function addproductattribute($request){
    ...
    if(Auth::user()->role_id==14){
        ...
        ->leftjoin('products_options_descriptions', 'products_options_descriptions.products_options_id', '=', 'products_options.products_options_id')
        ->where('products_options_descriptions.language_id', '=', $language_id)
        ->where('products_options.user_id', Auth::user()->id)
        ->get();
