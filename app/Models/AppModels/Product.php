...
class Product extends Model
{
  ...
  public static function allcategories($request)
  {
      ...
      if ($authenticate == 1) {
          ...
          if (count($categories) > 0) {
            ...
            //     $counrtuserids = DB::table('vendors_location')->where('location_name', $request->area_id)->groupBy('vendor_id')->where('deleted_at', 0)->get();
            ...
            //               ->whereIn('products.products_id', $productidsarray)
    ...
    function findvendorwithloc($areaid){
  ...
  public static function getallproducts($request)
  {
          ...
          if (!empty($request->filters)) {
              ...
              foreach ($request->filters as $filters_attribute) {
                  ...
                  if (!empty($request->categories_id)) {
                    ...
                    //  if(isset($request->area_id) && !empty($request->area_id)) {
                    //   $locationid = $request->area_id;
                    //   $product = DB::table('vendors_location')->where('location_name', '=', $locationid)->get();
                    //      //$product = DB::table('vendors_location')->where('id', '=', $locationid)->get();
                    //      $userporudctsarray = array(223123102);
                    //      foreach($product as $singleprd){
                    //         $userporudctsarray[] =$singleprd->vendor_id;  
                    //      }
                    ...
                    //         $userids[] =$singleprd->user_id;  
                    //      }
                    //     $data->whereIn('products.user_id', $userids);
                    //   }
                  
                   if(isset($request->vendor) && !empty($request->vendor)) {
                       
                       $data->whereIn('products.user_id', $request->vendor);
                   }
                          ...
                          if (!in_array($getProduct->products_id, $eliminateRecord)) {
                              ...
                              foreach ($products as $products_data) {
                                ...
                                $current_price = $products_data->products_price;
                                      ...
                                      $minorderprice = 0;
                                //       if (isset($request->area_id) && !empty($request->area_id)) {
                                //          // echo $products_data->products_id;die;
                                //         $proData = DB::table('products')->where('products_id',$products_data->products_id)->get();
                                //          // print_r($proData);die;
                                //          $vId = $proData[0]->user_id;
                                //          $vendordetail = DB::table('vendors')->where('user_id',$vId)->get();
                                //          $results =DB::table('vendors_location')->where('location_name',$request->area_id)->where('vendor_id',$vendordetail[0]->id)->get();
                                //         if($results->count()>0){
                                //             $shippingcharge = @$results[0]->shipping_fees;
                                //             $minorderprice = @$results[0]->min_order;
                                //         }else{
                                //             $shippingcharge = 0;
                                //             $minorderprice = 0;
                                //         }
                                ...
                                //   }
                                  ...
                                  $products_data->shipping_fees = $shippingcharge;
                                  $products_data->min_order =$minorderprice;
                                  
                                  $products_data->vendorname ="";
                                   $productvendor = DB::table('users')->where('id',$products_data->user_id)->get();
                                  if($productvendor->count()>0){
                                      $products_data->vendorname =$productvendor[0]->first_name;
                                  }
            ...
            //         $userids[] =$singleprd->user_id;  
            //      }
            //     $categories->whereIn('products.user_id', $userids);
            //   }
              if(isset($request->vendor) && !empty($request->vendor)) {
                       
                       $categories->whereIn('products.user_id', $request->vendor);
             }
              ...
              if (count($data) > 0) {
                ...
                //   }
                      ...
                      //echo $products_data->user_id;die;
                      $products_data->vendorname ="";
                       $productvendor = DB::table('users')->where('id',$products_data->user_id)->get();
                      if($productvendor->count()>0){
                          $products_data->vendorname =$productvendor[0]->first_name;
    ...
    public function getShipCharge($proId,$your_area){
     
        
         
         return array('shipping_fees'=>$shippingFee,'min_order'=>$min_order);
    }

  public static function notifyproduct($request)
  {
      $notify_products_id = $request->notify_products_id;
      $notify_customers_id = $request->notify_customers_id;
      ...
      if ($authenticate == 1) {
          ...
          DB::table('notify_products')->where([
              'notify_products_id' => $notify_products_id,
              'notify_customers_id' => $notify_customers_id,
          ])->delete();

          DB::table('notify_products')->insert([
              'notify_products_id' => $notify_products_id,
              'notify_customers_id' => $notify_customers_id,
              'date_liked' => $date_liked,
          ]);

          $response = DB::table('notify_products')->select('notify_products_id')->where('notify_customers_id', '=', $notify_customers_id)->get();          

          $responseData = array('success' => '1', 'product_data' => $response, 'message' => "Product will be notified.");
     ...
     // print_r($request->all());die;
      ...
      if ($authenticate == 1) {
          ...
          foreach ($products as $products_data) {
            ...
            $vendors = array();
            // if(isset($request->area_id) && !empty($request->area_id)){
            //       $product = DB::table('vendors_location')->where('location_name', '=', $request->area_id)->get();
            //          $userporudctsarray = array(223123102);
            //          foreach($product as $singleprd){
            //             $userporudctsarray[$singleprd->vendor_id] =$singleprd->vendor_id;  
            //          }
            //         $locationvendor =  $userporudctsarray;
          ...
          if (!empty($optionsIdArray)) {
              ...
              foreach ($optionsIdArray as $optionsIdArray) {
                  $option_name = DB::table('products_options')
                      ->leftJoin('products_options_descriptions', 'products_options_descriptions.products_options_id', '=', 'products_options.products_options_id')->select('products_options.products_options_id', 'products_options_descriptions.options_name as products_options_name', 'products_options_descriptions.language_id')->where('products_options.products_options_id', '=', $optionsIdArray)->get();
