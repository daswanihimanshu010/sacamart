...
class Location extends Model
{
    ...
    public static function citiesreq($request)
    {
        $consumer_data = array();
        $consumer_data['consumer_key'] = request()->header('consumer-key');
        $consumer_data['consumer_secret'] = request()->header('consumer-secret');
        ...
        if ($authenticate == 1) {
            ...
            $responseData = array('success' => '1', 'data' => $cities, 'message' => "Returned all cities.");
        } else {
            $responseData = array('success' => '0', 'data' => array(), 'message' => "Unauthenticated call.");
        }
        $response = json_encode($responseData);
    ...
    public static function areas($request)
    {
        $consumer_data = array();
        $consumer_data['consumer_key'] = request()->header('consumer-key');
        $consumer_data['consumer_secret'] = request()->header('consumer-secret');
        ...
        if ($authenticate == 1) {
            ...
            $areas = DB::table('locations')->where('city_id', $city_id)->get();

            $responseData = array('success' => '1', 'data' => $areas, 'message' => "Returned all areas.");
        } else {
            $responseData = array('success' => '0', 'data' => array(), 'message' => "Unauthenticated call.");
        }
        $response = json_encode($responseData);
    ...
    public static function zones($request)
    {
        ...
        if ($authenticate == 1) {
            ...
            $zones = DB::table('states')->where('country_id', $zone_country_id)->get();
