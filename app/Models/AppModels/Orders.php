...
public static function getrate($request)
{
  ...
  if($authenticate==1){
    ...
    foreach($request->products as $products_data){
      ...
      $products_desc = DB::table('products_description')->where('products_id', $products_data['products_id'])->get();

      if(count($products)>0){
        
        $tax_value = $products[0]->tax_rate/100*$final_price;
        $shipping_weight = json_decode($products_desc, true)[0]['shipping_weight'];
        $total_weight = $shipping_weight*$request->products[$index]['customers_basket_quantity'];
        $product_total_weight = $product_total_weight + $total_weight;
        $total_tax = $total_tax+$tax_value;
        $index++;
      }
    }

    $shipping_info = DB::table('locations')
        ->LeftJoin('shipping_weight_slabs', 'shipping_weight_slabs.shipping_zone_id','=','locations.shipping_zone_id')
        ->where('locations.id', $delivery_postcode)->where('shipping_weight_slabs.range_start','<=',$product_total_weight)->where('shipping_weight_slabs.range_end','>=',$product_total_weight)->first()->shipping_fee;

    if ($shipping_info == null) {
      $shipping_info = 0;
    }    

    /*$shippingFee = json_decode($shipping_info, true)[0]['shipping_fee']; */   

    if($total_tax>0){
      $total_tax = Orders::convertprice($total_tax, $requested_currency);
      $data['tax'] = $total_tax;
      $data['shippingFee'] = $shipping_info;
    }else{
      $data['tax'] = '0';
    }
  ...
  if($authenticate==1){
     foreach($request->products as $products){
      ...
      // print_r($request->guest_status);
         //die;
			...
			$billing_postcode            			=   $request->delivery_postcode;

			$billing = DB::table('states')->where('id', '=', $request->delivery_suburb)->first();

			if($billing){
				$billing_state            				=   $billing_suburb;
			}else{
				$billing_state            				=   'other';
			}
	...
	//						'updatedAt'=>$result->transaction->updatedAt->date,
        ...
        $payments_setting = Orders::payments_setting_for_hyperpay($request);
				...
				$payment_method = 'Hyperpay';
				$payment_status='success';
			}else if($payment_method == 'razropay'){
        $payments_setting = Orders::payments_setting_for_razorpay($request);
				$paymentMethodName = $payments_setting['RAZORPAY_KEY']->name;
				$payment_method = 'Razropay';
				$payment_status='success';
