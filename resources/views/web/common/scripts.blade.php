...
function getCities() {
    jQuery(function ($) {
      jQuery('#loader').show();
      var state_id = jQuery('#entry_zone_id').val();
      jQuery.ajax({
        beforeSend: function (xhr) { // Add this line
                xhr.setRequestHeader('X-CSRF-Token', $('[name="_csrfToken"]').val());
         },
        url: '{{ URL::to("/ajaxCities")}}',
        type: "POST",
        //data: '&country_id='+country_id,
        ...
        success: function (res) {
          ...
          showData.push("<option value='-1'>@lang('website.Other')</option>");
          jQuery("#entry_city").html(showData);
          jQuery('#loader').hide();
        },
      });
    });
...
function getLocations() {
    jQuery(function ($) {
      jQuery('#loader').show();
      var city_id = jQuery('#entry_city').val();
      jQuery.ajax({
        beforeSend: function (xhr) { // Add this line
                xhr.setRequestHeader('X-CSRF-Token', $('[name="_csrfToken"]').val());
         },
        url: '{{ URL::to("/ajaxLocations")}}',
        type: "POST",
        //data: '&country_id='+country_id,
        ...
        success: function (res) {
          ...
          for (i = 0; i < res.length; ++i) {
            var j = i + 1;
            showData[i] = "<option value='"+res[i].id+"'>"+res[i].location+"</option>";
          }
          showData.push("<option value='-1'>@lang('website.Other')</option>");
          jQuery("#entry_postcode").html(showData);
          jQuery('#loader').hide();
        },
      });
    });
...
jQuery(document).on('click', '.notify_me', function(e){
...
<script type="text/javascript">
    $(document).ready(function() {
        $('select[name="country_id"]').on('change', function() {
		    ...
		    if(countryIDs){
		        $.ajax({
		           type:"GET",
                   url:'{{ URL::to("get-state-list?country_id=")}}'+countryIDs,
		           success:function(res){               
		            if(res){
		                $(".state").empty();
         ...
         $('select[name="state_id"]').on('change', function() {
		    ...
		    if(stateIDs){
		        $.ajax({
		           type:"GET",
		           url:'{{ URL::to("/get-city-list?country_id=")}}'+stateIDs,
		           success:function(res){               
		            if(res){
                        var mycityhtml = '';
		                $(".city").empty();
         ...
         $('select[name="city_id"]').on('change', function() {
		    ...
		    if(stateIDs){
		        $.ajax({
		           type:"GET",
		           url:'{{ URL::to("/get-area-list?city=")}}'+stateIDs,
		           success:function(res){               
		            if(res){
                        var mycityhtml = '';
		                $.each(res,function(key,value){
                            mycityhtml +='<div class="col-md-4"><input  name="vendorcity[]" type="checkbox" class="mylocationcheck" value="'+key+'">&nbsp;&nbsp;'+value+'</div>'; 
		                    
                           
		                });
                        $('.displayarea').html(mycityhtml);
		                 $('#loader').hide();
    ...
    $(document).ready(function(){
        ...
        $(document).on("click", ".mylocationcheck", function(){
            var SlectedList = new Array();
             $('.mylocationcheck:checkbox:checked').each(function(){
            SlectedList.push($(this).val());
          });
           $('#selectedareas').val(SlectedList);  
            
        });
			
    });
    
    function displayqty(type,prdid){
        var qty = parseInt($('.myqtyprdlisting_'+prdid).val());
        if(type==1){
            if(qty < 5){
                $('.myqtyprdlisting_'+prdid).val(qty+1);
            }else{
                notificationWishlist("You have max added qty for this product");
            }
        }else{
            if(qty > 1){
                $('.myqtyprdlisting_'+prdid).val(qty-1);
            }else{
                notificationWishlist("Min qty available");
            }
        }
        
    }
    
    function addtocartlisting(prdid){
         var qty = parseInt($('.myqtyprdlisting_'+prdid).val());
         var price = $('.prdprice_'+prdid).val();
	 	var message;
	 	jQuery.ajax({
	 		url: '{{ URL::to("/addToCart")}}',
	 		headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
	 		type: "POST",
	 		data: {products_id:prdid,products_price:price,checkout:false,quantity:qty,'mycarttype':'updatequty'},

	 		success: function (res) {
	 			
	 					if(res == 'exceed'){
							notificationWishlist("Something Happened To Stock", "@lang('website.Ops! Product is available in stock But Not Active For Sale. Please contact to the admin')", "error");
	 					}
	 					else {
							...
							//notificationWishlist("Congrates!", "Product Added Successfully Thanks.Continue Shopping", "success");
