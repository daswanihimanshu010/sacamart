...
jQuery(document).on('click', '#load_products', function(e){
	jQuery('#ajax_loadmore').css('display','block');
	...
	var is_runajax = jQuery('#is_runajax').val();
	jQuery('#is_runajax').val(0);
	if(is_runajax==1){
    ...
    headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
		...
		success: function (res) {
			...
			jQuery('#ajax_loadmore').css('display','none');
		},
	});
	}
